...
const UserManagement = () => {
    ...
    stats?.growthData && stats.growthData.length > 0
      ? stats.growthData.map((item, i) => ({
          name:
            ...
            item.month ||
            monthsArr[
              (currentMonthIdx - (stats.growthData.length - 1 - i) + 12) % 12
            ],
          users: Math.max(
            1,
            clamp(item.users) ||
              Math.floor(
                (totalUsers || 2) * (0.3 + 0.7 * (i / stats.growthData.length)),
              ),
          ),
          projects: Math.max(
            1,
            clamp(item.projects) ||
              Math.floor(
                (totalProjects || 5) *
                  (0.3 + 0.7 * (i / stats.growthData.length)),
              ),
          ),
          arts: Math.max(
            1,
            clamp(item.arts) ||
              Math.floor(
                (totalArts || 3) * (0.3 + 0.7 * (i / stats.growthData.length)),
              ),
          ),
        }))
      : Array.from({ length: 9 }).map((_, i) => {
          const idx = (currentMonthIdx - 8 + i + 12) % 12;
          const val = [0.45, 0.75, 0.4, 0.45, 0.55, 0.2, 0.35, 0.7, 1.0][i];
          return {
            name: monthsArr[idx],
  ...
  const colorMap = {
    ...
    pink: {
      ...
      icon: "text-pink-600",
      num: "text-pink-700 dark:text-pink-300",
    },
  };
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-4 mb-10">
        {statCards.map(({ label, value, icon: Icon, color }) => {
          ...
          return (
            <div
              key={label}
              className="bg-white dark:bg-[#160327] p-5 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-md transition flex flex-col gap-3"
            >
              <div
                className={`h-11 w-11 rounded-xl ${c.bg} flex items-center justify-center`}
              >
              ...
              <div>
                ...
                <h4 className={`text-2xl font-black ${c.num} leading-none`}>
                  {formatNumber(value)}
                </h4>
              </div>
            </div>
          ...
          {isMounted && stats ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 font-medium italic">
              {!isMounted
                ? "Initialization..."
                : stats === null
                  ? "Failed to load analysis. Check server connection."
                  : "Calculating zig-zag trends..."}
            </div>
          )}
        </div>
      </div>
