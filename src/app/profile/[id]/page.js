...
export default function UserProfile() {
  ...
  const totalMusics = Math.abs(
    user.totalMusics ?? creations.musics?.length ?? 0,
  );
  const totalVideos = Math.abs(
    user.totalVideos ?? creations.videos?.length ?? 0,
  );
  const totalEvents = Math.abs(
    user.totalEvents ?? creations.events?.length ?? 0,
  );
      ...
      <div className="relative">
        ...
        <div className="h-56 md:h-72 relative overflow-hidden">
          {user.coverImageUrl ? (
            ...
            <div
              className="w-full h-full bg-gradient-to-r from-purple-600 via-violet-600 to-blue-600"
              aria-hidden="true"
            />
          )}
      ...
      <div className="px-4 mx-auto max-w-7xl pt-24">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white dark:bg-[#160327] p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
              ...
              <div className="flex items-center gap-6 border-y border-gray-100 dark:border-slate-700/50 py-5 my-6">
                <button
                  ...
                  className="group text-center flex-1 transition-all hover:scale-110 active:scale-95 outline-none border-none bg-transparent"
                >
                  <div className="text-xl font-black text-purple-600 dark:text-purple-400">
                    {followerCount}
                  </div>
                  <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest group-hover:text-purple-500">
                    Followers
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest group-hover:text-blue-500">
                    Following
                  </div>
                </button>
              </div>
              ...
              {(!currentUser ||
                ...
                >
                  {isFollowing ? (
                    <>
                      <FaUserMinus size={14} className="opacity-80" />{" "}
                      <span className="tracking-wide uppercase letter-spacing-wide">
                        Unfollow
                      </span>
                    </>
                  ) : (
                    <>
                      <FaUserPlus size={14} className="animate-pulse" />{" "}
                      <span className="tracking-wide uppercase letter-spacing-wide">
                        Follow Artist
                      </span>
                    </>
                ...
                <div className="border-t border-gray-100 dark:border-slate-700 pt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {displayBadges.map((badge, index) => (
                      <span
                        key={index}
                        className="flex items-center gap-1.5 px-3 py-1 bg-yellow-50 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400 rounded-full text-[10px] font-bold border border-yellow-200 dark:border-yellow-700/50"
                      >
                        <FaAward /> {badge}
                      </span>
                    ))}
            ...
            {/* Quick Stats */}
            <div className="bg-white dark:bg-[#160327] p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
              <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 text-center lg:text-left">
                Portfolio Stats
              </h4>
              ...
              {creations[activeTab]?.length > 0 ? (
                ...
                <div className="col-span-full py-20 text-center">
                  <p className="text-gray-400">
                    No {activeTab} shared by this artist yet.
                  </p>
                </div>
              )}
            </div>
