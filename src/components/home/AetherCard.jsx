...
const AetherCard = ({ item, type, butterflyImg }) => {
  ...
  const link = isVideo
    ? `/video/detail/${item._id}`
    : `/music/detail/${item._id}`;
  const mainImg = item.imageUrls?.[0] || item.image || item.thumbnail;
  const mainVideo =
    item.videoUrls?.[0] ||
    item.mediaFileUrls?.[0] ||
    ...
    >
      ...
      <div className="px-4 pb-4">
        ...
        <p className="text-slate-500 dark:text-purple-300 text-sm leading-relaxed mb-6 font-medium line-clamp-2">
          {item.description ||
            "Experience the highest quality content curated from our top tier creators worldwide."}
        </p>
        <div className="flex items-center justify-between text-[10px] font-bold uppercase text-slate-400 dark:text-purple-400">
          <span className="flex items-center gap-1.5">
            <FaCircleInfo className="text-primary" /> Enhanced Quality
          </span>
          <span className="text-primary px-3 py-1 bg-primary/10 rounded-full">
            {Math.max(0, item.reactions || item.likes?.length || 0)} Likes
          </span>
        </div>
      </div>
    </Link>
