...
const ArtsTable = () => {
  ...
  async function getAllArts(query) {
    ...
    try {
      ...
      const countRes = await artsAPI.getArtsCount(isAdmin ? { name: query.name } : { createdBy: user._id });
      setTotal(typeof countRes?.data === 'number' ? countRes.data : 0);
    } catch (error) {
       toast.error(error?.response?.data?.error || "Failed to fetch arts", { autoClose: 1500 });
    } finally {
      setLoading(false);
      ...
      <div className="flex flex-col px-6 py-6 space-y-4 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center flex-1 space-x-4">
          <div className="p-1 bg-primary/10 rounded-xl overflow-hidden shadow-inner flex items-center justify-center">
             <Image src={butterflyImg} width={36} height={36} alt="Arts" className="object-cover transition-transform hover:scale-110" />
          </div>
          <div>
            <h5 className="text-xl font-bold text-black dark:text-white">Arts Gallery</h5>
            <p className="text-sm text-gray-500 dark:text-gray-400">Curate and manage your artistic collection</p>
          </div>
        </div>
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${art.stock > 10 ? 'bg-green-500' : art.stock > 0 ? 'bg-yellow-500' : 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]'}`}></div>
                    <span className={`text-xs font-semibold ${art.stock === 0 ? 'text-red-500' : 'text-gray-600 dark:text-gray-400'}`}>
                      {art.stock ?? 0}
                    </span>
                  </div>
                </td>
                ...
                <td colSpan={7} className="px-6 py-20 text-center">
                  <div className="flex flex-col items-center">
                    <div className="w-20 h-20 bg-gray-50 dark:bg-[#160327] rounded-full flex items-center justify-center mb-4 border-2 border-dashed border-gray-200 dark:border-gray-700">
                      <Image src="/logo.png" width={40} height={40} alt="Empty" className="grayscale opacity-20" />
                    </div>
                    <h3 className="text-lg font-bold text-black dark:text-white">No Art Found</h3>
                    <p className="text-gray-500 max-w-[200px] mx-auto mt-1 text-sm italic">"Every artist was first an amateur."</p>
                    <Link href={`${ART_MANAGEMENT_ROUTE}/add`} className="mt-4 text-primary font-semibold text-sm hover:underline">
