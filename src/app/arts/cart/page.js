...
import { removeFromCart, fetchCart, decreaseQuantity, increaseQuantity } from '@/redux/cart/cartSlice';
...
const CartPage = () => {
  ...
  if (!isAuthenticated && items.length === 0) {
    return (
      <div className="container mx-auto py-16 px-4 text-center">
        ...
        <p className="text-gray-600 mb-8">Login to save your cart or browse our collection.</p>
        <Link href="/arts" className="bg-purple-600 text-white px-8 py-3 rounded-lg hover:bg-purple-700 transition shadow-md">
          Back to Arts
        </Link>
      </div>
    );
  ...
  if (loading) {
     return <div className="flex justify-center items-center h-64 text-purple-600 font-bold">Loading cart...</div>;
  }
  ...
  return (
    <div className="container mx-auto py-10 px-4 max-w-5xl transition-colors duration-500">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <Link href="/arts" className="inline-flex items-center gap-2 text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 mb-4 transition">
            <FaArrowLeft size={16} />
            Back to Arts
          </Link>
          <h1 className="text-4xl font-semibold text-black dark:text-white tracking-tight">Shopping Cart</h1>
        </div>
        <div className="text-right">
          <p className="text-gray-500 dark:text-purple-400/60 font-medium">Items in Cart</p>
          <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{items.length}</p>
        </div>
      </div>
      ...
      {items.length === 0 ? (
        <div className="bg-white dark:bg-[#160327] rounded-3xl border border-gray-100 dark:border-purple-900/30 p-12 text-center shadow-sm">
          ...
          <h2 className="text-2xl font-bold text-gray-800 dark:text-purple-100 mb-2">Cart is empty</h2>
          <Link href="/arts" className="text-purple-600 dark:text-purple-400 font-bold hover:underline">
            Go browse some art!
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-[#11041f] border border-gray-100 dark:border-purple-900/40 rounded-3xl p-6 shadow-xl sticky top-24 overflow-hidden">
              <h2 className="text-2xl font-semibold text-black dark:text-white mb-6">Order Summary</h2>
              <div className="space-y-4 mb-8">
                ...
                <div className="pt-4 border-t border-gray-100 dark:border-purple-900/30 flex justify-between">
                  ...
                  <span className="text-2xl font-semibold text-purple-600 dark:text-purple-400">Rs. {total.toLocaleString()}</span>
                </div>
              </div>
