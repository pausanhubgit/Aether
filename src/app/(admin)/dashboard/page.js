  ...
  useEffect(() => {
    async function getDashboardStats() {
      try {
        ...
        toast.error(error?.response?.data || "Failed to load stats", {
          autoClose: 1500,
        });
      }
    }
    ...
    <div className="px-4 mx-auto max-w-screen-2xl pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium">
            {isAdmin
              ? "System-wide performance and user analytics"
        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-3">
          <Link
            href="/"
            className="flex items-center gap-2 px-5 py-2.5 bg-gray-100 text-gray-700 dark:bg-[#160327] dark:text-gray-200 rounded-xl hover:bg-gray-200 dark:hover:bg-slate-600 transition-all font-bold text-sm shadow-sm"
          >
          ...
          {!isAdmin && (
            <>
              <Link
                href="/Art-management/add"
                className="flex items-center gap-2 px-5 py-2.5 bg-purple-600 !text-white rounded-xl hover:bg-purple-700 hover:shadow-purple-200/50 transition-all font-bold text-sm shadow-lg shadow-purple-600/20 dark:shadow-none"
              >
                <FaPlus className="text-xs" /> <span>Add Art</span>
              </Link>
              <Link
                href="/Music-management/add"
                className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 !text-white rounded-xl hover:bg-blue-700 hover:shadow-blue-200/50 transition-all font-bold text-sm shadow-lg shadow-blue-600/20 dark:shadow-none"
              >
                <FaPlus className="text-xs" /> <span>Add Music</span>
              </Link>
              <Link
                href="/Video-management/add"
                className="flex items-center gap-2 px-5 py-2.5 bg-green-600 !text-white rounded-xl hover:bg-green-700 hover:shadow-green-200/50 transition-all font-bold text-sm shadow-lg shadow-green-600/20 dark:shadow-none"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <Card
          ...
          icon={
            isAdmin ? (
              ...
              <FaMusic className="text-4xl text-blue-600" />
            )
          }
          value={
            isAdmin
              ? Math.max(0, stats?.totalUsers || 0)
              : Math.max(0, stats?.totalMusics || 0)
          }
        />
        <Card
          label="Total Videos"
        ...
        <Card
          ...
          value={
            isAdmin
              ? Math.max(0, stats?.totalEvents || 0)
              : Math.max(0, stats?.totalReactions || 0)
          }
        />
      </div>
        ...
        <div className="flex items-center gap-3 mb-6 md:mb-10">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 font-medium mt-0.5">
              Cumulative counts — actual database entries
            </p>
          </div>
        </div>
