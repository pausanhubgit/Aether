  ...
  if (!sorted.length) {
    return (
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8 text-center">
        <p className="text-lg font-semibold text-white/80">Your feed is empty.</p>
        <p className="mt-2 text-sm text-[var(--muted)]">Upload something from Dashboard to see it here.</p>
      </div>
    );
  }
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              ...
              <h3 className="mt-2 text-xl font-semibold text-white">{item.title || "Untitled"}</h3>
              {item.description ? (
