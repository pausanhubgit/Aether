...
const Action = ({ order, onUpdate }) => {
  ...
  const updateOrderStatus = async () => {
    ...
    try {
      ...
      if (estimatedDeliveryDate !== (order.estimatedDeliveryDate ? new Date(order.estimatedDeliveryDate).toISOString().split('T')[0] : "")) {
        ...
        info={
          <div className="pb-5">
            <div className="flex flex-col gap-4 max-w-xs mx-auto text-left">
              <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">Order Status</label>
              <select
              ...
              {(status === ORDER_STATUS_SHIPPED || status === ORDER_STATUS_CONFIRMED) && (
                <div className="animate-fade-in mt-2 p-4 bg-purple-500/5 rounded-2xl border border-purple-500/10">
                  <label className="text-[10px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-[0.2em] mb-2 block font-heading">Est. Delivery Date</label>
                  <input 
                    type="date" 
                  ...
                  />
                  <p className="text-[10px] text-gray-400 mt-2 font-medium italic">Required for customer tracking</p>
                </div>
              )}
            </div>
          </div>
