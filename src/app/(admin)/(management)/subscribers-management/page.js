...
import { FaBell, FaTrash, FaEnvelope, FaCalendarAlt, FaChartLine } from "react-icons/fa";
            ...
            <div className="bg-white dark:bg-[#160327] rounded-3xl border border-slate-200 dark:border-purple-900/40 overflow-hidden shadow-sm">
                <table className="w-full text-left">
                    <thead className="bg-slate-50 dark:bg-[#0f021b] border-b border-slate-200 dark:border-purple-900/40">
                        <tr>
                            <th className="px-6 py-4 text-sm font-bold text-gray-700 dark:text-slate-300 uppercase tracking-wider">Email Address</th>
                            <th className="px-6 py-4 text-sm font-bold text-gray-700 dark:text-slate-300 uppercase tracking-wider text-right">Subscribed At</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-purple-900/20">
                        {subscribers.length > 0 ? subscribers.map((sub) => (
                            <tr key={sub._id} className="hover:bg-slate-50 dark:hover:bg-[#0f021b] transition-colors">
                                <td className="px-6 py-5">
                                    <div className="flex items-center gap-3">
                                        <div className="p-2 bg-primary/5 text-primary rounded-lg">
                                            <FaEnvelope size={14} />
                                        </div>
                                        <span className="text-gray-800 dark:text-slate-200 font-medium">{sub.email}</span>
                                    </div>
                                </td>
                            ...
                            <tr>
                                <td colSpan={2} className="px-6 py-20 text-center text-gray-500 dark:text-purple-300">
                                    No subscribers yet.
                                </td>
                            </tr>
                        )}
