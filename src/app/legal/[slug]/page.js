      ...
      <aside className="lg:w-64 flex-shrink-0">
        <div className="sticky top-24 space-y-2">
          ...
          {navItems.map((item) => (
            ...
            >
              <span
                className={`${slug === item.id ? "!text-white" : ""} text-lg`}
              >
                {item.icon}
              </span>
              <span
                className={`${slug === item.id ? "!text-white font-bold" : "font-semibold"} text-sm`}
              >
                {item.label}
              </span>
              {slug === item.id && (
                <FiChevronRight className="ml-auto text-xs !text-white" />
              )}
            </Link>
          ))}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10 pb-10 border-b border-gray-100 dark:border-purple-900/20">
            <div>
              ...
              <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 text-sm">
                ...
                <span>
                  Last Updated: <strong>{page.lastUpdated}</strong>
                </span>
              </div>
            </div>
          </div>
