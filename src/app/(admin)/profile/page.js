...
const VideoThumb = ({
  ...
  const vidVideo =
    vid.videoUrls?.[0] || (typeof vid.media === "string" ? vid.media : null);
    ...
    >
      ...
      <button
        onClick={(e) => {
          ...
          e.stopPropagation();
          setActiveDropdown(
            activeDropdown === `video-${vid._id}` ? null : `video-${vid._id}`,
          );
        }}
      ...
      {activeDropdown === `video-${vid._id}` && (
        <div className="absolute top-12 right-4 w-32 bg-white dark:bg-[#160327] rounded-xl shadow-2xl border border-gray-100 dark:border-slate-700 py-1 z-30 overflow-hidden">
          <button
            onClick={(e) => handleEditItem("video", vid._id, e)}
            className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-purple-50 dark:hover:bg-purple-900/30 text-left"
          >
            <FaEdit /> Edit
          </button>
          <button
            onClick={(e) => handleDeleteItem("video", vid._id, e)}
      ...
      <Link href={`/video/detail/${vid._id}`}>
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-6 flex flex-col justify-end z-10 cursor-pointer">
          ...
          <p className="text-green-400 text-xs font-semibold uppercase tracking-widest flex items-center gap-2">
            <FaVideo /> Watch Now
          </p>
        </div>
      </Link>
...
const MusicTrack = ({
  ...
  const musicMedia =
    music.videoUrls?.[0] ||
    music.audioUrls?.[0] ||
    (typeof music.media === "string" ? music.media : null);
    ...
    >
      {musicMedia ? (
        <video
          ...
          loop
          autoPlay={
            isHovering || music.hide === false || music.hide === "false"
          }
        />
      ) : musicImage ? (
        <img
      ...
      {activeDropdown === `music-${music._id}` && (
        <div className="absolute top-12 right-4 w-32 bg-white dark:bg-[#160327] rounded-xl shadow-2xl border border-gray-100 dark:border-slate-700 py-1 z-30 overflow-hidden">
          <button
            onClick={(e) => handleEditItem("music", music._id, e)}
            className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-purple-50 dark:hover:bg-purple-900/30 text-left"
          >
            <FaEdit /> Edit
          </button>
          <button
            onClick={(e) => handleDeleteItem("music", music._id, e)}
            className="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 text-left"
          >
            <FaTrash /> Delete
          </button>
      ...
      <Link href={`/music/detail/${music._id}`}>
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-6 flex flex-col justify-end z-10 cursor-pointer pointer-events-auto">
          ...
          <p className="text-blue-400 text-xs font-semibold uppercase tracking-widest flex items-center gap-2">
            <FaMusic /> Listen Now
          </p>
        </div>
      </Link>
...
const ProfilePage = () => {
  ...
  const handleDeleteItem = async (type, id, e) => {
    ...
    try {
      if (type === "art") {
        ...
        setCreations((prev) => ({
          ...prev,
          arts: prev.arts.filter((item) => item._id !== id),
        }));
      } else if (type === "music") {
        await apiInstance.delete(`/api/music/${id}`);
        setCreations((prev) => ({
          ...prev,
          musics: prev.musics.filter((item) => item._id !== id),
        }));
      } else if (type === "video") {
        await apiInstance.delete(`/api/video/${id}`);
        setCreations((prev) => ({
          ...prev,
          videos: prev.videos.filter((item) => item._id !== id),
        }));
      } else if (type === "event") {
        await apiInstance.delete(`/api/events/${id}`);
        setCreations((prev) => ({
          ...prev,
          events: prev.events.filter((item) => item._id !== id),
        }));
      }
  ...
  const openFollowList = (type) => {
    ...
    const list =
      type === "followers" ? fullProfile.followers : fullProfile.following;
          ...
          <div className="p-8 sm:p-10 border-b border-gray-100 dark:border-slate-700 relative">
            ...
            <div className="absolute top-8 right-8">
              <div className="relative">
                ...
                >
                  <FaCog
                    className={`text-xl ${showSettings ? "rotate-90" : ""} transition-transform duration-300`}
                  />
                </button>
                {showSettings && (
                  <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-[#160327] rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-700 p-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    <button
                      onClick={() => {
                        if (
                          ...
                          toast.error(
                            "Access Denied: Only admins can access the management panel.",
                            {
                              position: "top-center",
                              autoClose: 3000,
                    ...
                    <Link
                      ...
                      className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-green-50 dark:hover:bg-green-900/20 text-gray-700 dark:text-gray-200 transition group"
                    >
              ...
              <div className="flex-1 text-center md:text-left mb-2">
                <h1 className="text-4xl font-semibold text-black dark:text-white mb-2 flex items-center justify-center md:justify-start gap-3">
                  ...
                  {user?.roles?.some((role) =>
                    ["ADMIN", "MERCHANT", "CREATOR", "VENDOR"].includes(
                      role.toUpperCase(),
                    ),
                  ) && (
                    <span
                      className="p-1 bg-blue-500 rounded-full text-white shadow-lg shadow-blue-200"
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-4 text-gray-500 dark:text-gray-400 font-medium">
                  <span className="flex items-center gap-1.5">
                    ...
                    {user?.address?.city || user?.city || "Unknown Location"}
                  </span>
                  <span className="px-3 py-1 bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400 rounded-lg text-xs font-bold uppercase tracking-wider">
                    {user?.roles?.find((r) => r.toUpperCase() === "ADMIN")
                      ? "Admin"
                      : user?.roles?.[0] || "User"}
                  </span>
                  {user?.roles?.some(
                    ...
                    <span className="flex items-center gap-1.5 px-3 py-1 bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400 rounded-lg text-xs font-semibold uppercase tracking-widest border border-amber-200 dark:border-amber-700 shadow-sm shadow-amber-200/50">
                      <FaUserShield className="text-amber-500 animate-pulse" />{" "}
                      Official Admin
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center justify-center md:justify-start gap-4 mt-6">
                  <button
                    ...
                    className="group text-center flex flex-col items-center gap-1 transition-all hover:scale-105 active:scale-95 outline-none border-none"
                  >
                    <span className="text-xl font-black text-purple-600 dark:text-purple-400 leading-none">
                      {fullProfile?.followers?.length || 0}
                    </span>
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest group-hover:text-purple-500">
                      Followers
                    </span>
                  </button>
                  ...
                  >
                    <span className="text-xl font-black text-blue-600 dark:text-blue-400 leading-none">
                      {fullProfile?.following?.length || 0}
                    </span>
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest group-hover:text-blue-500">
                      Following
                    </span>
                  </button>
                </div>
              ...
              <div>
                <section className="mb-8">
                  ...
                  <p className="text-lg text-gray-600 dark:text-gray-300 leading-relaxed bg-gray-50 dark:bg-[#160327]/30 p-8 rounded-2xl border border-gray-100 dark:border-slate-700 italic">
                    "
                    {user?.bio ||
                      "No biography shared yet. This user is a mystery!"}
                    "
                  </p>
                </section>
                ...
                <section className="mb-8">
                  ...
                  <div className="space-y-8">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                          {creations.arts.map((art) => {
                            const artImage =
                              art.imageUrls?.[0] || art.image || art.thumbnail;
                            return (
                              <div
                                key={art._id}
                                className="group relative aspect-square rounded-2xl overflow-hidden bg-gray-100 border border-gray-100 dark:border-slate-700"
                              >
                                {artImage ? (
                                  <img
                                ...
                                {activeDropdown === `art-${art._id}` && (
                                  <div className="absolute top-10 right-2 w-32 bg-white dark:bg-[#160327] rounded-xl shadow-2xl border border-gray-100 dark:border-slate-700 py-1 z-30 overflow-hidden">
                                    ...
                                    <button
                                      onClick={(e) =>
                                        handleDeleteItem("art", art._id, e)
                                      }
                                      className="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 text-left"
                                    >
                                      <FaTrash /> Delete
                                ...
                                <Link href={`/arts/${art._id || art.id}`}>
                                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4 z-10 cursor-pointer pointer-events-auto">
                                    ...
                                    <p className="text-purple-300 text-[10px] font-semibold uppercase">
                                      View Details →
                                    </p>
                                  </div>
                                </Link>
                    ...
                    {creations.musics?.length > 0 && (
                      <div>
                        <h4 className="flex items-center gap-2 text-sm font-bold text-gray-400 uppercase tracking-widest mb-4 ml-1">
                          Music Library
                        </h4>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {creations.events.map((evt) => (
                            <div
                              key={evt._id}
                              className="p-6 bg-gradient-to-br from-purple-50 to-white dark:from-slate-800 dark:to-slate-900 rounded-[2rem] border border-purple-100 dark:border-slate-700 shadow-sm relative overflow-hidden group"
                            >
                              <div className="absolute top-0 right-0 p-4 bg-purple-600 text-white rounded-bl-[2rem] text-[10px] font-semibold uppercase tracking-tighter">
                                Hosted
                              </div>
                              ...
                              <div className="absolute bottom-6 right-6 z-20">
                                ...
                                {activeDropdown === `event-${evt._id}` && (
                                  <div className="absolute bottom-12 right-0 w-32 bg-white dark:bg-[#160327] rounded-xl shadow-2xl border border-gray-100 dark:border-slate-700 py-1 z-30 overflow-hidden mb-2">
                                    <button
                                      onClick={(e) =>
                                        handleEditItem("event", evt._id, e)
                                      }
                                      className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-purple-50 dark:hover:bg-purple-900/30 text-left"
                                    >
                                      <FaEdit /> Edit
                                    ...
                                    <button
                                      onClick={(e) =>
                                        handleDeleteItem("event", evt._id, e)
                                      }
                                      className="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 text-left"
                                    >
                                      <FaTrash /> Delete
                              ...
                              <h5 className="text-xl font-semibold text-black dark:text-white mb-2 pr-12 leading-tight">
                                {evt.title}
                              </h5>
                              <div className="flex items-center gap-4 text-xs font-bold text-gray-500 mb-4">
                                <span className="flex items-center gap-1">
                                  <FaCalendarAlt className="text-purple-500" />{" "}
                                  {format(new Date(evt.startDate), "MMM dd")}
                                </span>
                                <span className="flex items-center gap-1">
                              ...
                              <Link
                                href={`/events/${evt._id}`}
                                className="text-xs font-semibold text-purple-600 uppercase tracking-widest hover:text-purple-800 transition-colors"
                              >
                                Manage Event →
                              </Link>
                            </div>
                    ...
                    {/* Empty State */}
                    {!creations.arts?.length &&
                      !creations.musics?.length &&
                      ...
                      !creations.events?.length && (
                        <div className="text-center py-20 bg-gray-50 dark:bg-[#160327]/20 rounded-[3rem] border-2 border-dashed border-gray-200 dark:border-slate-700">
                          ...
                          <p className="text-gray-500 mt-2">
                            Start creating and hosting to build your audience!
                          </p>
                        </div>
                      )}
                  </div>
                ...
                <section className="mt-16 pt-16 border-t border-gray-100 dark:border-slate-800">
                  <div className="flex items-center justify-between mb-10">
                    <div>
                      <h3 className="text-3xl font-semibold text-black dark:text-white tracking-tight">
                        Participating In
                      </h3>
                  ...
                  {registrations?.length > 0 ? (
                        ...
                        >
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <h4 className="text-lg font-semibold text-black dark:text-white truncate group-hover:text-purple-600 transition-colors">
                                {reg.eventId?.title}
                              </h4>
                              <p className="text-sm text-gray-500 font-medium mt-1">
                                ...
                                {reg.eventId?.startDate
                                  ? format(
                                      new Date(reg.eventId.startDate),
                                      "MMM dd, yyyy",
                                    )
                                  : "TBD"}
                              </p>
                              <p className="text-xs text-purple-600 font-bold uppercase tracking-widest mt-2 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                                View Details{" "}
                    ...
                    <div className="text-center py-16 bg-gray-50 dark:bg-[#160327]/20 rounded-[3rem] border-2 border-dashed border-gray-200 dark:border-slate-700">
                      ...
                      <Link
                        href="/events"
                        className="inline-block mt-4 text-purple-600 font-semibold text-sm hover:underline"
                      >
                        Browse Events →
                      </Link>
                    </div>
                  ...
                  <div className="bg-red-50 dark:bg-red-950/10 rounded-[2.5rem] p-8 md:p-10 border border-red-100 dark:border-red-900/20">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                      <div className="text-center md:text-left">
                        ...
                        <p className="text-sm text-red-500/70 font-medium max-w-md">
                          Once you delete your account, there is no going back.
                          All your arts, music, and videos will be removed from
                          the Aether arena.
                        </p>
                      </div>
                      <button
            ...
            <div className="max-h-[400px] overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-purple-900">
              {showFollowModal.users.length > 0 ? (
                showFollowModal.users.map((f) => {
                  ...
                  const uName = isPopulated
                    ? f.name || f.username || "Merchant"
                    : "User";
                  ...
                  return (
                    <Link
                      ...
                      href={`/profile/${uId}`}
                      onClick={() =>
                        setShowFollowModal({ type: null, users: [] })
                      }
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-white font-bold text-sm truncate group-hover:text-purple-300">
                          {uName}
                        </div>
                        <div className="text-purple-400/50 text-[10px] tracking-widest font-bold uppercase truncate">
                          @{uHandle}
                        </div>
                      </div>
                      <FaChevronRight
                        size={12}
                        className="text-purple-500/30 group-hover:text-purple-400 flex-shrink-0"
                      />
                    </Link>
                  );
                })
              ) : (
                <div className="py-20 text-center text-purple-400/30 text-sm">
                  No {showFollowModal.type} yet.
                </div>
              )}
            </div>
          </div>
