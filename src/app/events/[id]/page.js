...
export default function EventDetailPage() {
  ...
  const isAlreadyRegistered =
    ...
    event?.registrations?.some(
      (reg) =>
        reg.userId === user._id ||
        reg.userId?._id === user._id ||
        reg.userId === user.id ||
        reg.userId?._id === user.id,
    );
    ...
    <div className="min-h-screen bg-white dark:bg-[#0d0118] pb-24 selection:bg-purple-500 selection:text-white">
        ...
        <div className="container mx-auto px-4 max-w-7xl relative z-10">
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-10">
            ...
            <div className="w-full lg:w-auto shrink-0">
              <button
                onClick={() =>
                  !isAlreadyRegistered && setShowRegisterModal(true)
                }
              ...
              <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 bg-gray-50 dark:bg-white/5 rounded-3xl border border-gray-100 dark:border-white/5">
                  ...
                  <p className="text-sm text-gray-500 font-medium">
                    All submissions must be original work. Collaboration is
                    allowed only where explicitly stated.
                  </p>
                </div>
                <div className="p-6 bg-gray-50 dark:bg-white/5 rounded-3xl border border-gray-100 dark:border-white/5">
                  ...
                  <p className="text-sm text-gray-500 font-medium">
                    Entries will be judged by community legends and industry
                    experts based on creativity and execution.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-white dark:bg-[#160327]/40 rounded-[3rem] p-8 md:p-14 shadow-sm border border-gray-100 dark:border-white/5 backdrop-blur-sm mt-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                  {event.registrations.map((reg, idx) => (
                    <div
                      key={idx}
                      className="flex flex-col items-center p-4 bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/5 group hover:bg-white dark:hover:bg-white/10 transition-all"
                    >
                      ...
                      <p className="text-xs font-bold text-gray-800 dark:text-white text-center line-clamp-1">
                        {reg.name}
                      </p>
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="relative group p-1 rounded-[3rem] bg-gradient-to-br from-amber-400 via-orange-500 to-red-600 shadow-2xl">
              <div className="bg-white dark:bg-[#160327] rounded-[2.8rem] p-10 flex flex-col items-center text-center overflow-hidden relative">
                ...
                <h3 className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.3em] mb-2 leading-none">
                  Bounty & Rewards
                </h3>
            ...
            <div className="bg-white dark:bg-[#160327]/40 rounded-[3rem] p-10 shadow-sm border border-gray-100 dark:border-white/5 backdrop-blur-sm text-center">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-8 border-b border-gray-100 dark:border-white/5 pb-4">
                Tournament Master
              </h3>
              <div className="flex flex-col items-center">
                <div className="relative">
                  <div className="w-24 h-24 rounded-[30%] border-4 border-white dark:border-[#0d0118] overflow-hidden bg-purple-50 shadow-2xl relative z-10 group-hover:rotate-6 transition-transform">
                    {event.creatorUserId?.profileImageUrl ? (
                      <img
                        src={event.creatorUserId.profileImageUrl}
                        alt="Host"
                        className="w-full h-full object-cover"
                      />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-purple-500 to-blue-600 font-black text-white text-3xl">
                        {event.creatorUserId?.name?.charAt(0) ||
                          event.creatorUserId?.username?.charAt(0) ||
                          "H"}
                      </div>
                    )}
                  </div>
                ...
                <div className="mt-6">
                  <Link href={`/profile/${event.creatorUserId?._id}`}>
                    <p className="font-black text-gray-900 dark:text-white text-2xl tracking-tight leading-tight hover:text-primary transition-colors cursor-pointer">
                      {event.creatorUserId?.name ||
                        event.creatorUserId?.username ||
                        "Legendary Host"}
                    </p>
                  </Link>
                  <p className="text-purple-600 dark:text-purple-400 text-xs font-black uppercase tracking-widest mt-2">
                    Arena Curator
                  </p>
                </div>
      ...
      {showRegisterModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 overflow-y-auto py-10">
          <div
            className="fixed inset-0 bg-[#0d0118]/80 backdrop-blur-xl"
          ...
          <div className="relative bg-white dark:bg-[#160327] w-full max-w-xl rounded-[4rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] p-10 md:p-14 transform scale-100 transition-all border border-gray-100 dark:border-white/10 overflow-hidden">
            ...
            <div className="mb-10">
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-3 font-medium text-lg leading-relaxed">
                Joining the legendary showdown:{" "}
                <span className="text-purple-600 font-black underline underline-offset-8 decoration-purple-600/30">
                  {event.title}
                </span>
              </p>
            </div>
            ...
            <form onSubmit={handleRegisterEvent} className="space-y-6">
              <div>
                <label className="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-3 ml-2">
                  Your Identity
                </label>
                <div className="relative">
              ...
              <div className="pt-6">
                ...
                <p className="text-center text-[10px] text-gray-400 font-bold mt-6 uppercase tracking-widest">
                  AETHER ART HUB • OFFICIAL COMPETITION
                </p>
              </div>
            </form>
          </div>
