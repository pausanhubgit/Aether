            ...
            <div key={order._id} className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm hover:shadow-md transition">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 border-b border-gray-50 dark:border-purple-900/40 pb-4">
                <div className="w-full md:w-auto overflow-hidden">
                  ...
                  <p className="text-sm font-mono font-bold text-purple-600 dark:text-purple-400 break-all">{order.orderNumber || order._id}</p>
                </div>
                 <div className="flex flex-wrap gap-2 sm:gap-4 items-center">
                    <div className={`flex items-center gap-2 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-bold whitespace-nowrap flex-shrink-0 ${getStatusClass(order.status)}`}>
                      {getStatusIcon(order.status)}
                      <span className="capitalize">{order.status}</span>
                    </div>
                    <p className="text-xs sm:text-sm font-bold text-gray-400 dark:text-gray-500 whitespace-nowrap">{new Date(order.createdAt).toLocaleDateString()}</p>
                 </div>
              </div>
              ...
              <div className="flex flex-wrap gap-4 mb-6">
                {order.orderItems.map((item, idx) => {
                  ...
                  const art = (item.artId && typeof item.artId === 'object') ? item.artId : {};
                  const imageUrl = art.imageUrls?.[0] || art.image || null;
                  return (
                    <div key={item._id || idx} className="flex items-center gap-3 bg-gray-50 p-2 rounded-2xl border border-gray-100">
                      <div className="h-12 w-12 relative rounded-lg overflow-hidden border border-white">
                        {imageUrl ? (
                          ...
                          <div className="h-full w-full bg-purple-100 flex items-center justify-center text-purple-400 text-xs font-bold">Art</div>
                        )}
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-gray-800 truncate max-w-[120px]">{art.title || art.name || 'Artwork'}</p>
                        <p className="text-[10px] text-gray-400 font-bold">Qty: {item.quantity}</p>
                        {art.price && <p className="text-[10px] text-purple-500 font-bold">Rs. {Number(art.price).toLocaleString()}</p>}
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center bg-purple-50 p-4 rounded-2xl gap-4 mt-4">
                 <div className="text-center sm:text-left w-full sm:w-auto">
                  ...
                  {order.status === 'pending' ? (
                    ...
                    >
                      {trackingOrderId === order._id ? 'Close Tracking' : 'Track Order'}
                    </button>
                  ) : null}
              </div>
              ...
              {trackingOrderId === order._id && (
                <div className="mt-6 border-t border-gray-100 dark:border-purple-900/30 pt-6 animate-fade-in">
                  <h3 className="text-sm font-bold text-gray-800 dark:text-white mb-6 tracking-tight">Tracking Progress</h3>
                  <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-0">
                    ...
                    {(() => {
                        const statusWeights = { 'pending': 0, 'confirmed': 1, 'shipped': 2, 'delivered': 3 };
                        const orderStatusWeight = statusWeights[order.status?.toLowerCase()] ?? -1;
                        
                        const calcDaysBetween = (start, end) => Math.ceil((end - start) / (1000 * 60 * 60 * 24));
                        const orderStart = new Date(order.createdAt);
                        // Example dynamic/manual tracking assumption: +3 days from shipped to delivered
                        let expectedDeliveryDate = new Date(orderStart.getTime() + (5 * 24 * 60 * 60 * 1000));
                        if (order.estimatedDeliveryDate) expectedDeliveryDate = new Date(order.estimatedDeliveryDate);
                        
                        const daysLeft = calcDaysBetween(new Date(), expectedDeliveryDate);
                        const isLate = daysLeft < 0;

                        const stages = [
                          { label: 'Pending', icon: FaClock, active: orderStatusWeight >= 0 },
                          { label: 'Confirmed (Paid)', icon: FaCheckCircle, active: orderStatusWeight >= 1 },
                          { label: 'Shipped', icon: FaTruck, active: orderStatusWeight >= 2 },
                          { label: 'Delivered', icon: FaBox, active: orderStatusWeight >= 3 }
                        ];

                        return (
                          <>
                            <div className="hidden md:block absolute top-[18px] left-[5%] h-1 bg-purple-500 transition-all rounded-full z-0" style={{ width: `${(Math.max(0, orderStatusWeight) / 3) * 90}%` }}></div>
                            
                            {stages.map((stage, idx) => (
                              <div key={stage.label} className="flex flex-row md:flex-col items-center gap-4 md:gap-2 relative z-10 w-full md:w-auto">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-4 border-white dark:border-[#0d0118] transition-colors ${stage.active ? 'bg-purple-600 text-white' : 'bg-gray-100 dark:bg-[#160327] text-gray-400'}`}>
                                  <stage.icon size={14} />
                                </div>
                                <div className="text-left md:text-center flex-grow">
                                  <p className={`text-xs font-bold ${stage.active ? 'text-gray-900 dark:text-white' : 'text-gray-400'}`}>{stage.label}</p>
                                  {(stage.label === 'Shipped' && (orderStatusWeight === 1 || orderStatusWeight === 2)) && (
                                    <span className={`text-[10px] sm:text-xs font-semibold px-2 py-0.5 rounded-xl block mt-1 break-words w-auto max-w-[120px] md:mx-auto text-center ${isLate ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-700'}`}>
                                      {isLate ? 'Delayed Delivery' : `Expected in ${daysLeft} days`}
                                    </span>
                                  )}
                                  {(stage.label === 'Delivered' && orderStatusWeight === 3) && (
                                    <span className="text-[10px] font-bold text-green-500 block mt-1">Order Complete</span>
                                  )}
                                </div>
                              </div>
                            ))}
