...
const CheckoutPage = () => {
    ...
    try {
      const orderData = {
        orderItems: items.map(item => {
          ...
          const artId = (item.artId && typeof item.artId === 'object')
  ...
  return (
    <div className="container mx-auto py-10 px-4 max-w-4xl">
      <h1 className="text-3xl font-semibold text-black dark:text-white mb-8">Checkout</h1>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-[#160327] p-6 rounded-2xl border border-gray-100 dark:border-purple-900/30 shadow-sm">
            <h2 className="text-xl font-bold flex items-center gap-2 mb-4 dark:text-white">
              <FaMapMarkerAlt className="text-purple-600 dark:text-purple-400" /> Shipping Details
            </h2>
          ...
          <div className="bg-white dark:bg-[#160327] p-6 rounded-2xl border border-gray-100 dark:border-purple-900/30 shadow-sm text-center">
            <h2 className="text-xl font-bold flex items-center justify-center gap-2 mb-4 dark:text-white">
              <FaCreditCard className="text-purple-600 dark:text-purple-400" /> Payment
            </h2>
            <p className="text-gray-500 dark:text-gray-400 mb-4 text-sm sm:text-base">We currently support Cash on Delivery and Online Payment via Khalti.</p>
            <div className="flex justify-center gap-4">
               <div className="border-2 border-purple-600 dark:border-purple-500 p-4 rounded-xl flex flex-col items-center gap-2 bg-purple-50 dark:bg-purple-900/20">
                 <FaMobileAlt size={24} className="text-purple-600 dark:text-purple-400" />
                 <span className="font-bold text-xs dark:text-purple-200 text-purple-900">Standard Billing</span>
               </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-gray-900 dark:bg-black/40 dark:border dark:border-purple-900/50 text-white p-6 sm:p-8 rounded-3xl shadow-2xl">
            ...
            <div className="space-y-4 mb-8 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {items.map((item, index) => {
                ...
                return (
                  <div key={artId ? `${artId}-${index}` : index} className="flex justify-between items-center bg-white/5 hover:bg-white/10 p-3 rounded-xl border border-white/5 transition-colors">
                    <div>
                      <p className="font-bold text-sm truncate max-w-[150px]">{art.title || art.name || 'Art'}</p>
                      <p className="text-white/50 text-xs">Qty: {item.quantity || 1}</p>
                    </div>
                    <p className="font-semibold text-purple-400">Rs. {Number(price).toLocaleString()}</p>
                  </div>
                );
              })}
            </div>
            ...
            <p className="flex items-start sm:items-center justify-center gap-2 text-white/40 text-[10px] sm:text-xs mt-4 mt-top text-center sm:text-left">
              ...
              <span>You'll choose your payment method (Khalti / Stripe / COD) on the next step.</span>
            </p>
          </div>
        </div>
      </form>
